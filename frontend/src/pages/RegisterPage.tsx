...
const RegisterPage = () => {
    ...
    return (
      <div className="flex flex-col items-center justify-center px-10 py-12 mx-auto md:h-[1000px] lg:py-0">
        <Link to="/register" className="flex items-center mb-6 text-2xl font-semibold text-gray-900 dark:text-white">
          <img className="rounded-full w-20 h-auto mr-4" 
                    src="/src/assets/Screenshot_1.png"
          alt="logo"/>
          <span>Ishavo Store</span>
        </Link>
        <div className="w-full md:w-[400px] lg:w-[500px] bg-slate-300 rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
          <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
            ...
            <form className="space-y-4 md:space-y-6" onSubmit={handleSubmit}>
              ...
              <div>
                <label htmlFor="re-password" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Repetir contraseña</label>
                <input 
                value={re_password}
                required
            ...
            {handleMatch() ? false : <p className="text-sm font-medium text-red-500">Contraseñas no coinciden</p>}
              ...
              <p className="text-sm font-light text-gray-500 dark:text-gray-400">
                ya tienes una cuenta ? <Link to={'/login'} className="font-medium text-primary-600 hover:underline dark:text-primary-500">Inicia sesion</Link>
              </p>
            </form>
          </div>
        </div>
